...
for the project */
export async function installDeps(framework, tailwind, builder, prettier, pkgManager, targetDir) {
  const pm = installCmds[pkgManager];
  const { deps, devDeps } = getDependencies(framework, tailwind, builder, prettier);
